          ...
          {filteredProducts.map((product, index) => (
            <div key={index} className="border border-slate-500 p-4 rounded-lg">
              ...
              <div className="flex flex-col gap-2 mt-3">
                <button onClick={() => addToCart(product)} className="border rounded-md px-4 py-2 text-[14px] text-white bg-yellow-900">
                  Add to cart
                </button>
                <button onClick={() => addToWishlist(product)}  className="border rounded-md px-4 py-2 text-[14px] text-yellow-900 border-yellow-800">
                  WishList
                </button>
              </div>
            </div>
